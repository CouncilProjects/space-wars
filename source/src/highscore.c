...
{
    FILE *hsfile;
    if(hsfile=fopen("textures/data.dat","rb"))//if there is a file with the highscores get them from there
    {
...
void saveTable()
{
    FILE *hsfile=fopen("textures/data.dat","wb");
    if(hsfile==NULL)
    {
        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION,"Error saving","Progress migh not have been saved",NULL);
        exit(1);
    }
